...
export class ContactEffects {
  ...
  @Effect() addContact$ = this.actions$.pipe(
    ofType('ADD_CONTACT'),
    mergeMap(action =>
      from(this.firestore.collection("users").doc(this.auth.auth.currentUser!.uid).collection("contacts").add(action.payload.data)).pipe(
  ...
  @Effect() loadContacts$ = this.actions$.pipe(
    ...
    mergeMap(action =>
      this.firestore.collection("users").doc(this.auth.auth.currentUser!.uid).collection("contacts").get().pipe(
  ...
  @Effect() updateContact$ = this.actions$.pipe(
    ...
    mergeMap(action =>
      from(this.firestore.collection("users").doc(this.auth.auth.currentUser!.uid).collection("contacts").doc(action.payload.id).set(action.payload.data)).pipe(
  ...
  @Effect() deleteContact$ = this.actions$.pipe(
    ...
    mergeMap(action =>
      from(this.firestore.collection("users").doc(this.auth.auth.currentUser!.uid).collection("contacts").doc(action.payload.id).delete()).pipe(
